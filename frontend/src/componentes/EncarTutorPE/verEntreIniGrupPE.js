...
class VerEntreIniGrupPE extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            readDatos:[]
        }
    }//Metodo para obtener los datos generales del alumno, nombres, apellidos, pertenecientes a la entrevista inicial 
    componentDidMount(){
        let grupo = this.props.match.params.grupo;
        const Url = "http://localhost:3000/apis/list/encargado/PE/group/all/"+grupo;
        axios.get(Url)
        .then(res=>{
            if(res.data.success){
        ...
        return this.state.readDatos.map((datos, index)=>{
            return(
                <ul class="list-group list-group-horizontal">
                    ...
                    <li class="list-group-item text-center col-md-1">
                        <center>
                            <Link title="Ver resultados" to={'/verEntreIniIndividual/'+this.props.match.params.id+"/"+this.props.match.params.grupo+"/"+datos.idAlumnos+"/"+datos.tipo+"/"+this.props.match.params.volver} class="btn btn-primary">
    ...
    }//Función para re direccinar a multiples vistas 
    volver(){
        if(this.props.match.params.volver === "1"){
            window.location.replace("/verEntreIniJyPE/"+this.props.match.params.id);
        }else{
            window.location.replace("/filtrarFecha/"+this.props.match.params.id+"/"+this.props.match.params.volver);
        }
    }
}

export default VerEntreIniGrupPE;