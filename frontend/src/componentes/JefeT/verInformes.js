...
class verInformes extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            readDatos:[],
    ...
    }//Metodo para obtener los informes de los resultados por grupos, del examen ceneval 
    componentDidMount(){
        const Url = "http://localhost:3000/apis/list/Result/For/groups";
        axios.get(Url)
        .then(res=>{
            const datos = res.data.data;
            this.setState({readDatos:datos});
        });//Metodo para obtener los años de la api
        ...
        return(
            <div style={ sectionStyle } className="estatic">
                ...
                <div class="container-fluid">
                    <h4 class="text-center text-white">INFORMES DEL EXAMEN CENEVAL POR GRUPOS</h4>
                    <br></br>
    ...
    }//Funión para mostrar los datos divididos por grupo 
    read(){
        return this.state.readDatos.map((datos, index)=>{
            return(
                <ul class="list-group list-group-horizontal">
                    ...
                    <li class="list-group-item text-center col-md-1">
                        <center>
                            <Link title="Ver grupo" class="btn btn-primary" to={"/verIfrmGrupExII/"+this.props.match.params.id+"/"+datos.grupoTutores+"/"+this.state.campoAno}>
    ...
    }//Función para filtrar por medio de años 
    readAnos(){
    ...
    }//Función para ir a la vista de filtrar por año
    filtrar(){
        window.location.replace("/verResIndiExamII/"+this.props.match.params.id+"/"+this.state.campoAno);
    }
}
